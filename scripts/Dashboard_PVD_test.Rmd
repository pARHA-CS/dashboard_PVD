...
df <- df %>% mutate(TYPEQU = recode(TYPEQU, !!!typequ_mapping))
df_full <- df_full %>% mutate(TYPEQU = recode(TYPEQU, !!!typequ_mapping))
```
...
renderValueBox({
  ...
  nb_equipements <- df %>% filter(LIBCOM == ville_cible, DOM == "D") %>% nrow()
...
renderValueBox({
  ville_cible <- input$ville
  df_autres_villes <- df_full %>% filter(LIBCOM != ville_cible, DOM == "D")

  moyenne_autres_villes <- df_autres_villes %>%
    group_by(LIBCOM) %>%
    summarise(count = n()) %>%
    summarise(moyenne = mean(count))
...
renderLeaflet({
  ...
  if (nrow(df_sante_ville) == 0) {
    ...
    leaflet(df_sante_ville) %>%
      ...
      addCircleMarkers(
        data = df %>% filter(LIBCOM == ville_cible, DOM == "D", TYPEQU != typequ_cible),
        ~LONGITUDE, ~LATITUDE,
        radius = 6,
        color = "blue",
        fillOpacity = 0.7,
